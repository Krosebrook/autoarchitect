...
import { db, storage } from '../services/storageService';
...
const ProfileView: React.FC = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      localStorage.setItem('aa_user_profile', JSON.stringify(profile)); // Keep for legacy theme check in App.tsx
      setIsSaved(true);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in pb-20">
      ...
      <div className="p-10 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-[3rem] flex items-center justify-between group overflow-hidden relative">
        ...
        <button 
          onClick={() => { if(confirm('Wipe all local architect data?')) { db.delete().then(() => window.location.reload()); } }}
